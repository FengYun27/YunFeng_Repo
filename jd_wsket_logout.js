 ...
 * JD_WSCK_LOGOUT=ALL 退出所有 CK 有效的账户
 */

const $ = new Env("注销WSKEY")
...
!(async () => {
    ...
    let JD_WSCK_LOGOUT = process.env['JD_WSCK_LOGOUT'].toUpperCase()
    if (!JD_WSCK_LOGOUT) {
        JD_WSCK_LOGOUT = ''
    }
    if (JD_WSCK_LOGOUT !== 'ALL') {
        if (JD_WSCK_LOGOUT.indexOf('&') !== -1) {
            JD_WSCK_LOGOUT = JD_WSCK_LOGOUT.split('&')
        } else if (JD_WSCK_LOGOUT.indexOf("\n") !== -1) {
            JD_WSCK_LOGOUT = JD_WSCK_LOGOUT.split("\n")
        } else if (JD_WSCK_LOGOUT) {
            JD_WSCK_LOGOUT = [JD_WSCK_LOGOUT]
        } else {
            JD_WSCK_LOGOUT = []
        }
    }
    if (!JD_WSCK_LOGOUT || !JD_WSCK_LOGOUT.length) {
